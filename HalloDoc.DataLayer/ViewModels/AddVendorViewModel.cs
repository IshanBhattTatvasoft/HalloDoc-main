...
public class AddVendorViewModel
{
    ...
    [RegularExpression(@"[a-zA-Z]{2,}$", ErrorMessage = "Business name must contain letters only")]
    ...
    [RegularExpression(@"^[1-9]\d{9}$", ErrorMessage = "Phone number must contain 10 digits only")]
    ...
    [RegularExpression(@"^[1-9]\d{9}$", ErrorMessage = "Business contact must contain 10 digits only")]
